...
function get_Route_FilePath(ctx,data)
{
    ...
    if(typeof(data) === 'object')
    {
        ...
        {
            data.forEach((item)=>{
                // console.log('----------',item,ctx.request.url,ctx.request.url.includes(item.route))
...
function  static(data){
    return async function static (ctx,next){
        ...
        // console.log('-------',route,route,ctx.request.url.match(route))
        const matchArr = ctx.request.url.match(router)
        if(matchArr&&matchArr[0].length>1)
        {
